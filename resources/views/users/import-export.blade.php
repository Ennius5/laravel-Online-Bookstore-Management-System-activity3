    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-semibold text-gray-900">Book Import / Export</h2>
                    <p class="mt-1 text-sm text-gray-600">Bulk upload books via Excel/CSV or export your catalogue in multiple formats.</p>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form action="{{ route('admin.books.export') }}" method="GET" class="space-y-4">

                        {{-- Format --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Export Format</label>
                            <div class="flex gap-3">
                                @foreach(['xlsx' => '📊 XLSX', 'csv' => '📄 CSV', 'pdf' => '📋 PDF'] as $value => $label)
                        ...
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Processed</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($importLogs as $log)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3 font-mono text-xs text-gray-700">{{ $log->filename }}</td>
                                        <td class="px-4 py-3">
                                            <span class="px-2 py-0.5 rounded-full text-xs font-medium
                                                @if($log->status === 'completed') bg-green-100 text-green-800
                                                @elseif($log->status === 'failed') bg-red-100 text-red-800
                                                @elseif($log->status === 'processing') bg-blue-100 text-blue-800
                                                @else bg-yellow-100 text-yellow-800
                                                @endif">
                                                {{ ucfirst($log->status) }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-3 text-gray-600">{{ $log->processed_rows }}</td>
